import { CompositeRule, IsNotNullOrUndefined, StringIsNotNullEmptyRange } from '@valencia/rules-engine';
...
export class NotificationIsValidRule extends CompositeRule {
  ...
  constructor(name: string, message: string, target: Notification, isDisplayable: boolean = false) {
  ...
  private configureRules() {
    ...
    this.target.messages.forEach((item) => {
      this.rules.push(
        new StringIsNotNullEmptyRange('NotificationIsValid', 'The message item is not valid. Must be within 1 and 300 characters.', item, 1, 300)
      );
    });
  }
}
